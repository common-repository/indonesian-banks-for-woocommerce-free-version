...
class WC_Gateway_bri extends WC_Gateway_BACS {
	function __construct() {
		$this->id = "bank_bri";
		$this->method_title = __( "Bank BRI", 'woocommerce' );
		$this->method_description = __( "Here you can setup offline payment method using Bank BRI transfer", 'woocommerce' );
		$this->title = __( "Transfer Bank BRI", 'woocommerce' );
		$this->icon = plugins_url( 'assets/logo-bri.png',__FILE__);
   ...
   add_action( 'woocommerce_thankyou_bank_bri', array( $this, 'thankyou_page' ) );
   add_action( 'woocommerce_email_before_order_table', array( $this, 'email_instructions' ), 10, 3 );

 	// BACS account fields shown on the thanks page and in emails
   $this->account_details = get_option( 'woocommerce_bank_bri_accounts',
    array(
        array(
...
public function init_form_fields() {
    $this->form_fields = array(
        'enabled' => array(
            ...
            'label'     => __( 'Enable <strong>Bank BRI</strong> Transfer Payment Method', 'woocommerce' ),
            'type'      => 'checkbox',
            'default'   => 'yes',
        ),
        'title' => array(
        ...
        'description' => array(
            ...
            'default'   => __( 'Use this form to describe how this payment method would work. e.g. Gunakan metode pembayaran transfer Bank BRI.', 'woocommerce' ),
            // 'css'       => 'max-width:350px;'
        ),
        'instructions' => array(
...
public function process_payment( $order_id ) {
    ...
    $order->update_status( 'on-hold', __( 'Waiting for Payment through Bank BRI', 'woocommerce' ) );
...
public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
    ...
    if ( ! $sent_to_admin && 'bank_bri' === $order->payment_method && $order->has_status( 'on-hold' ) ) {
        if ( $this->instructions ) {
