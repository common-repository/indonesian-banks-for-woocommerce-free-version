...
class WC_Gateway_bni extends WC_Gateway_BACS {
	function __construct() {
		...
		$this->method_description = __( "Here you can setup offline payment method using Bank BNI transfer", 'woocommerce' );
 ...
 public function init_form_fields() {
    $this->form_fields = array(
        ...
        'description' => array(
            ...
            'default'   => __( 'Use this form to describe how this payment method would work. e.g. Gunakan metode pembayaran transfer Bank BNI.', 'woocommerce' ),
            // 'css'       => 'max-width:350px;'
        ),
        'instructions' => array(
...
public function process_payment( $order_id ) {
    ...
    $order->update_status( 'on-hold', __( 'Waiting for Payment through Bank BNI', 'woocommerce' ) );
...
public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
    ...
    if ( ! $sent_to_admin && 'bank_bni' === $order->payment_method && $order->has_status( 'on-hold' ) ) {
        if ( $this->instructions ) {
